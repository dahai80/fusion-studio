...
extension IPCClient {
    ...
    func restartMLX(model: String = "") async throws -> [String: Any] {
        return try await call(method: RPCMethod.mlxRestart, params: ["model": model])
    }

    func mlxHealth() async throws -> [String: Any] {
        return try await call(method: RPCMethod.mlxHealth)
    }

    func mlxSetModel(model: String) async throws -> [String: Any] {
        return try await call(method: RPCMethod.mlxSetModel, params: ["model": model])
    }

    // MARK: - Graph Extended

    func graphGet(graphId: String) async throws -> [String: Any] {
        return try await call(method: RPCMethod.graphGet, params: ["graph_id": graphId])
    }

    // MARK: - Knowledge

    func knowledgeSearch(query: String, limit: Int = 5) async throws -> [String: Any] {
        return try await call(method: RPCMethod.knowledgeSearch, params: ["query": query, "limit": limit])
    }
    ...
    func knowledgeIngest(content: String, scope: String = "default", metadata: [String: Any]? = nil) async throws -> [String: Any] {
        ...
        return try await call(method: RPCMethod.knowledgeIngest, params: params)
    }
    ...
    func knowledgeDelete(entryId: String) async throws -> [String: Any] {
        return try await call(method: RPCMethod.knowledgeDelete, params: ["entry_id": entryId])
    }
    ...
    func knowledgeList(scope: String = "", limit: Int = 100) async throws -> [String: Any] {
        return try await call(method: RPCMethod.knowledgeList, params: ["scope": scope, "limit": limit])
    }

    func knowledgeCount(scope: String = "") async throws -> [String: Any] {
        return try await call(method: RPCMethod.knowledgeCount, params: ["scope": scope])
    }
    ...
    func plannerCreatePlan(task: String, context: String = "", files: [String]? = nil) async throws -> [String: Any] {
        ...
        return try await call(method: RPCMethod.plannerCreatePlan, params: params)
    }

    func plannerGetPlan(planId: String) async throws -> [String: Any] {
        return try await call(method: RPCMethod.plannerGetPlan, params: ["plan_id": planId])
    }

    func plannerApprovePlan(planId: String) async throws -> [String: Any] {
        return try await call(method: RPCMethod.plannerApprovePlan, params: ["plan_id": planId])
    }

    func plannerRejectPlan(planId: String, reason: String = "") async throws -> [String: Any] {
        return try await call(method: RPCMethod.plannerRejectPlan, params: ["plan_id": planId, "reason": reason])
    }

    func plannerExecuteStep(planId: String, stepId: String) async throws -> [String: Any] {
        return try await call(method: RPCMethod.plannerExecuteStep, params: ["plan_id": planId, "step_id": stepId])
    }

    func plannerExecutePlan(planId: String) async throws -> [String: Any] {
        return try await call(method: RPCMethod.plannerExecutePlan, params: ["plan_id": planId])
    }

    func plannerListPlans(status: String = "") async throws -> [String: Any] {
        return try await call(method: RPCMethod.plannerListPlans, params: ["status": status])
    }

    func plannerCancelPlan(planId: String) async throws -> [String: Any] {
        return try await call(method: RPCMethod.plannerCancelPlan, params: ["plan_id": planId])
    }
    ...
    func ragQuery(query: String, config: [String: Any] = [:], model: String = "", systemPrompt: String = "") async throws -> [String: Any] {
        ...
        return try await call(method: RPCMethod.ragQuery, params: params)
    }

    func ragRetrieve(query: String, config: [String: Any] = [:]) async throws -> [String: Any] {
        var params: [String: Any] = ["query": query]
        if !config.isEmpty { params["config"] = config }
        return try await call(method: RPCMethod.ragRetrieve, params: params)
    }
    ...
    func memoryStore(content: String, scope: String = "default", tags: String = "", importance: Int = 5, metadata: [String: Any]? = nil, tier: String = "") async throws -> [String: Any] {
        ...
        return try await call(method: RPCMethod.memoryStore, params: params)
    }

    func memoryRecall(query: String, scope: String = "", limit: Int = 10, minImportance: Int = 0, tier: String = "") async throws -> [String: Any] {
        return try await call(method: RPCMethod.memoryRecall, params: ["query": query, "scope": scope, "limit": limit, "min_importance": minImportance, "tier": tier])
    }

    func memoryListRecent(scope: String = "", limit: Int = 20, minImportance: Int = 0, tier: String = "") async throws -> [String: Any] {
        return try await call(method: RPCMethod.memoryListRecent, params: ["scope": scope, "limit": limit, "min_importance": minImportance, "tier": tier])
    }

    func memoryGet(entryId: String) async throws -> [String: Any] {
        return try await call(method: RPCMethod.memoryGet, params: ["entry_id": entryId])
    }

    func memoryDelete(entryId: String) async throws -> [String: Any] {
        return try await call(method: RPCMethod.memoryDelete, params: ["entry_id": entryId])
    }

    func memoryDeleteScope(scope: String) async throws -> [String: Any] {
        return try await call(method: RPCMethod.memoryDeleteScope, params: ["scope": scope])
    }

    func memoryCount(scope: String = "", tier: String = "") async throws -> [String: Any] {
        return try await call(method: RPCMethod.memoryCount, params: ["scope": scope, "tier": tier])
    }

    // MARK: - Safety

    func safetyCheck(content: String, context: String = "") async throws -> [String: Any] {
        return try await call(method: RPCMethod.safetyCheck, params: ["content": content, "context": context])
    }

    func safetyEvaluateAction(category: String, content: String = "", context: String = "") async throws -> [String: Any] {
        return try await call(method: RPCMethod.safetyEvaluateAction, params: ["category": category, "content": content, "context": context])
    }

    func safetyApproveAction(actionId: String) async throws -> [String: Any] {
        return try await call(method: RPCMethod.safetyApproveAction, params: ["action_id": actionId])
    }

    func safetyRejectAction(actionId: String) async throws -> [String: Any] {
        return try await call(method: RPCMethod.safetyRejectAction, params: ["action_id": actionId])
    }

    func safetyGetPendingActions() async throws -> [String: Any] {
        return try await call(method: RPCMethod.safetyGetPendingActions)
    }
    ...
    func safetyAddPolicy(category: String, description: String = "", defaultLevel: String = "L2", requiresDiff: Bool = false) async throws -> [String: Any] {
        return try await call(method: RPCMethod.safetyAddPolicy, params: ["category": category, "description": description, "default_level": defaultLevel, "requires_diff": requiresDiff])
    }

    // MARK: - Template

    func templateList(category: String = "") async throws -> [String: Any] {
        return try await call(method: RPCMethod.templateList, params: ["category": category])
    }

    func templateGet(templateId: String) async throws -> [String: Any] {
        return try await call(method: RPCMethod.templateGet, params: ["template_id": templateId])
    }
    ...
    func templateInstantiate(templateId: String, variables: [String: String]? = nil) async throws -> [String: Any] {
        ...
        return try await call(method: RPCMethod.templateInstantiate, params: params)
    }
    ...
    func deployExport(graphId: String, format: String = "json", filepath: String = "", withServer: Bool = true, port: Int = 8000) async throws -> [String: Any] {
        ...
        params["port"] = port
        return try await call(method: RPCMethod.deployExport, params: params)
    }

    func deployImport(filepath: String) async throws -> [String: Any] {
        return try await call(method: RPCMethod.deployImport, params: ["filepath": filepath])
    }

    func deployListFormats() async throws -> [String: Any] {
        return try await call(method: RPCMethod.deployListFormats)
    }
