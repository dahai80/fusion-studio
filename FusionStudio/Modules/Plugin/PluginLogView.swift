...
struct PluginLogView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            HStack(spacing: 8) {
                ...
                TextField("插件ID", text: $pluginFilter).textFieldStyle(.plain).padding(6).background(theme.inputBackground).cornerRadius(6).frame(width: 120)
                Spacer()
            ...
            }.padding(.horizontal, 16).padding(.vertical, 8).background(theme.backgroundSecondary)
            ScrollView {
                LazyVStack(alignment: .leading, spacing: 2) {
                    ForEach(filteredEntries) { entry in
                        HStack(alignment: .top, spacing: 8) {
                            ...
                            Text(entry.message).font(.system(size: 11, design: .monospaced)).foregroundStyle(theme.textPrimary).lineLimit(3)
                            Spacer()
    ...
    private func levelColor(_ level: String) -> Color {
        switch level { case "error": theme.danger; case "warn": .yellow; case "debug": .gray; default: theme.accent }
    }
}
